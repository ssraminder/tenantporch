/**
 * Split generators for Alberta lease documents.
 * Produces separate LeaseDocumentContent for each document type:
 * - Lease Agreement (sections 1-12 + additional terms)
 * - Schedule A (property details)
 * - Schedule B (tenant identification)
 *
 * Reuses the same interfaces and helpers from alberta.ts.
 */
...
/**
 * Generate the main Lease Agreement content (sections 1-12 + additional terms).
 * Excludes Schedule A and Schedule B sections.
 */
export function generateLeaseAgreementContent(
  lease: LeaseData,
  ...
  const leaseTypeLabel =
    lease.lease_type === "fixed" ? "Fixed Term" : "Month-to-Month (Periodic)";

  const ownersList =
    owners && owners.length > 0
      ? owners
      : [{ ...landlord, designation: "owner" }];
...
/**
 * Generate Schedule A — Property Details as a standalone document.
 */
export function generateScheduleAContent(
  property: PropertyData
): LeaseDocumentContent {
  const fullAddress = `${property.address_line1}${property.address_line2 ? `, ${property.address_line2}` : ""}, ${property.city}, ${property.province_state} ${property.postal_code}`;
...
/**
 * Generate Schedule B — Tenant Identification as a standalone document.
 */
export function generateScheduleBContent(
  tenants: PersonData[]
): LeaseDocumentContent {
  return {
    templateVersion: "AB-2026-1.0",
