...
/**
 * Reset ALL signing state for a lease so the landlord can run the flow again.
 *
 * This is destructive and intended for re-running the signing flow (e.g. after
 * a test run). It will:
 *   1. Cancel every signing request on the lease (any status).
 *   2. Reset every rp_lease_documents row to draft (clears signing_request_id,
 *      signed_pdf_url, signed_offline_at, signed_offline_by).
 *   3. Reset rp_leases.signing_status to draft and clear sent_for_signing_at.
 *   4. Optionally remove any rp_documents rows that were generated by a signing
 *      completion (categories: lease / schedule_a / schedule_b).
 *
 * Participant rows + audit log entries are kept for traceability.
 */
export async function resetLeaseSignatures(
  leaseId: string,
  options?: { wipeSignedPdfs?: boolean }
) {
  try {
    const supabase = await createClient();
    const {
    ...
    // Platform-admin only: resetting signatures discards a tenant's
    // executed signature, which a landlord must not be able to do.
    if (!isPlatformAdmin(rpUser.email)) {
      return {
        success: false,
        error: "This action is restricted to TenantPorch platform admins.",
      };
    }
    ...
    const wipePdfs = options?.wipeSignedPdfs ?? true;

    // 1. Cancel all signing requests on this lease
    ...
    if (requestIds.length > 0) {
      ...
      const auditRows = requestIds.map((id) => ({
        ...
        metadata: { reset_by: rpUser.id, lease_id: leaseId },
      }));
      await supabase.from("rp_signing_audit_log").insert(auditRows);
    }

    // 2. Reset all lease document rows back to draft
    await supabase
      .from("rp_lease_documents")
      .update({
    ...
    // 4. Optionally wipe generated signed PDFs from the documents repository
    ...
    revalidatePath(`/admin/properties/${(lease as any).property_id}`);

    return {
      success: true,
      cancelledRequests: requestIds.length,
      removedDocuments: removedDocCount,
    };
  } catch (error) {
    return {
      success: false,
